...
2. В зависимости от того, на каком языке программирования/в какой среде разработки вы выполняли лабораторную работу, создайте файл `.gitignore`, чтобы исключить из репозитория временные файлы и папки. Примеры шаблонов файлов `.gitignore` для различных языков программирования и сред разработки можно найти на [GitHub](https://github.com/github/gitignore).
Например, при создании C# проекта в Visual Studio генерируется директории `bin/` и `obj/`, которые содержат скомпилированные файлы и не должны попадать в репозиторий. 
Для C# проекта файл `.gitignore` находится в Dotnet.gitignore файле.
3. Добавьте файлы в отслеживаемые/готовые для коммита командой `git add *`, сделайте коммит. Если вы делаете коммит на данном устройстве впервые (и не представлялись Git ранее), то вас попросят ввести имя пользователя и email (читайте подсказки в терминале).
4. При переходе в пустой репозиторий на GitHub или GitVerse вам будут предложены команды для отправки вашей локальной копии. Ваша задача состоит в том, чтобы их выполнить.
![Example Gitverse](./gitverse.png)
5. При использовании GitVerse на шаге `git push` вас попросят ввести логин и пароль (пароль вы создали на шаге 3 для GitVerse). Введите его.
   Для Github вы уже авторизовались через GitHub CLI на шаге 4 в разделе Github.
6. Ссылку на ваш репозиторий отправьте в [форму](https://forms.gle/6gDznwbSihqhcqju5).

Если оказалось непосильно, то можете попробовать проделать все шаги с помощью графического клиента Git (см. ниже).
