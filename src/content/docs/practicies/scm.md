...
3. Выберите любую лабораторную работу из семестра, которую вы готовы опубликовать. Инициализируйте локальный репозиторий Git командой `git init` (Команду запускать из папки с вашей лабораторной работой!). Задайте имя пользователя и email.
```sh
...
5. Свяжите локальный репозиторий с удалённым репозиторием на GitHub. Команда для этого выглядит так:
   ...
   ```
6. Добавьте индекс командой `git add *`, сделайте коммит.
7. Единоразово переименуйте основную ветку в `main` командой `git branch -M main`.
8. Отправьте изменения в GitHub командой `git push -u origin main`.
9. Через веб-интерфейс GitHub создайте файл `README.md`. Опишите в нем, что этот репозиторий содержит.
10. Получите эти изменения в локальный репозиторий командой `git pull`.
11. Ссылку на ваш репозиторий отправьте в [форму](https://forms.gle/6gDznwbSihqhcqju5).
