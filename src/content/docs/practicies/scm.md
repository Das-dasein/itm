...
2. Создайте репозиторий на GitHub. Назовите его `itm_2025`.
3. В удобном месте на компьютере создайте папку `itm_2025`. Инициализируйте локальный репозиторий Git командой `git init` (Команду запускать из папки `itm_2025`). Задайте имя пользователя и email.
```sh
4. Скачайте и установите GitHub CLI.
...
5. Свяжите локальный репозиторий с удалённым репозиторием на GitHub. Команда для этого выглядит так:
   ...
   ```
6. Добавьте в папку `itm_2025` схему алгоритма из первой практики (формат на ваш выбор: картинка, PDF).
7. Добавьте индекс командой `git add *`, сделайте коммит.
8. Единоразово переименуйте основную ветку в `main` командой `git branch -M main`.
9. Отправьте изменения в GitHub командой `git push -u origin main`.
10. Через веб-интерфейс GitHub создайте файл `README.md`. Опишите в нем, что этот репозиторий содержит домашние работы студента <....> по по предмету "Введение в специальность".
11. Получите эти изменения в локальный репозиторий командой `git pull`.
12. Ссылку на ваш репозиторий отправьте в [форму](https://forms.gle/6gDznwbSihqhcqju5).
